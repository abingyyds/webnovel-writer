...
- 多用户账号：支持模型账号登录，也保留平台用户名密码。
- 模型网关：后端复用模型账号会话，自动创建或复用用户调用密钥，再调用 OpenAI 兼容接口；前端读取模型后直接选择使用。
...
MODEL_GATEWAY_BASE_URL=http://model-gateway.railway.internal:8080
```

`MODEL_GATEWAY_BASE_URL` 是模型网关管理/API 服务地址，不需要手动加 `/v1`；本平台在模型列表和生成请求时会自动调用 `${MODEL_GATEWAY_BASE_URL}/v1/...`。
...
把 `model-gateway` 换成你的 Railway 模型网关服务名（如果不同）。这个 `.railway.internal` 地址只能由后端服务访问，浏览器不能直连；本平台所有登录、模型列表和生成请求都会从后端代理出去。
